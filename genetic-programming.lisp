...
(defun tree-random-recombine (male female)
  "Return two new trees generated by randomly combining trees MALE and FEMALE."
  (let* ((male-index    (random (tree-size male)))
         (female-index  (random (tree-size female)))
         (male-fragment (tree-ref male male-index))
         (female-fragment (tree-ref female female-index)))
    (values (tree-combine female female-index male-fragment)
            (tree-combine male male-index female-fragment))))
